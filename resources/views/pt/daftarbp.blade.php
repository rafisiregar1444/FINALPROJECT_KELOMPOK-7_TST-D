...
<head>
    ...
    <title>Daftar Badan Penyelenggara | LLDIKTI Wilayah</title><!--begin::Primary Meta Tags-->
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary"><!--begin::App Wrapper-->
                            ...
                            <div class="card card-info card-outline mb-4"><!--begin::Header-->
                                <div class="card-header">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <h5 class="mb-0">Daftar Badan Penyelenggara</h5>
                                    </div>
                                </div>
                                <div class="card-header">
                                ...
                                <div class="card-body">
                                    <table id="table1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            @foreach ($bp as $index=>$list)
                                                <tr class="align-middle">
                                                    <td class="text-center">{{$index +1}}</td>
                                                    <td class="text-left">{{ $list->nama_yys_pt }}</td>
                                                    <td class="text-left"></td>
                                                    <td class="text-left">{{ $list->kd_kl_pddikti }}</td>
                                                    ...
                                                    <td class="text-center">
                                                        <form action="{{ route('daftarbpstore', ['id_pts' => $pt->id_pts]) }}" method="POST">
                                                            @csrf
                                                            <input type="hidden" name="id_bp" value="{{ $list->id_yys_pt }}">
                                                            <input type="hidden" name="id_pts" value="{{ $pt->id_pts }}">
                                                            <button type="submit" class="btn btn-primary ajukan" id="ajukan"
                                                                    data-idpts="{{ $pt->id_pts }}" 
                                                                    data-idbp="{{ $list->id_yys_pt }}">
                                                                Pilih
                                                            </button>
                                                        </form>
                                                    </td>
    ...
    <script>
        ...
        $('.ajukan').click(function(e) {
                ...
                let idbp = $(this).data('idbp');

                // Setel nilai input tersembunyi dalam formulir
                form.find('input[name="id_pts"]').val(idpts);
                form.find('input[name="id_bp_pts"]').val(idbp);

                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: 'Anda akan memilih {{ $pt->nama_pts }} ke perguruan tinggi dari {{ $list->nama_yys_pt }}',
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
