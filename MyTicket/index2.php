...
<head>
    ...
    <title>MyTicket | Pemesanan Tiket Spesial Lomba</title><!--begin::Primary Meta Tags-->
                            ...
                            <div class="card mb-6">
                                <div class="card-header">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <h3 class="mb-0">Pemesanan Tiket Spesial Lomba</h3>
                                        <style>
                                            .custom-orange {
    ...
    $(document).ready(function() {
        $('#orderForm').on('submit', function(e) {
            ...
            if (username && ticketId) {
                $.ajax({
                    type: 'POST',
                    url: 'check_voucherapi.php',  
                    data: JSON.stringify({ username: username, ticketId: ticketId }), 
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 'success') {
                            if (response.message.toLowerCase().includes('diskon')) {
                                $('#orderResult').addClass('alert-success').html(response.message + '<br>Harga setelah diskon: Rp ' + response.discounted_price);
                            } else {
                                $('#orderResult').addClass('alert-info').html(response.message + '<br>Harga tiket: Rp ' + response.price);
                            }
                        } else {
                            $('#orderResult').addClass('alert-danger').html(response.message);
                        }
                    },
                    error: function() {
                        $('#orderResult')
